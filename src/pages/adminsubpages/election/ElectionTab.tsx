...
import { useAvailableOrganizations, useCreateElection, useDeleteElection, useElections } from '../../../hooks/queries/useElection';
...
export default function ElectionTab() {
  ...
  //console.log(electionsQuery.data);
  
  const sortedReverse = electionsQuery.data?.sort((a:any, b:any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  const availableQueary = useAvailableOrganizations()
  ...
  const updateOrganization = async (e:any) => {
    ...
    await axiosPrivate.patch(`/election/single/${modifyId}`, updateElecData)
    .then((response) => {
        //console.log('Success:', response.data);
        message.open({
          key,
  ...
  const onClick: MenuProps['onClick'] = async ({ key }) => {
    ...
    await axiosPrivate.patch('election/connect-org-election', connectPosition)
    .then((response) => {
        //console.log('Success:', response.data);
        message.open({
          key: messageKey,
  ...
  const items: MenuProps['items'] = availableQueary.data?.length === 0 
    ? [{
    ...
    <div className='Election bg-white dark:bg-[#303030] rounded-b-lg shadow-md'>
        ...
        {electionsQuery.status === 'error' || electionsQuery.data?.[0]?.error === 'Network Error'
            ...
            : sortedReverse.length === 0 
                ...
                : <div className="grid items-center gap-5">
                  {sortedReverse.map((elec:DataType, index: any) =>(
      ...
      <Drawer title="Modify" onClose={onCloseMultiDrawer} open={openMulti}>
        <div className='pop-medium'>
          ...
          <label className='pb-1 opacity-80 mt-8'>Election Name</label>
